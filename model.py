...
t = pulp.OrderedDict({i: f[i-1] for i in set_I})
...
# Constraints
for j in set_J:       
    Model += pulp.lpSum(x[i,j] for i in set_I) >= 1  #3#
    Model += pulp.lpSum(x[i,j] for i in set_I)  <=2   #4#
    Model += pulp.lpsum(x[i,j] for i in set_I)  <= 1  #5#
    Model += (pulp.lpSum(x[i,j] for i in set_I)) -(1) == y1 #9#
for i in set_I:        
    Model += pulp.lpSum(y1*(0.5)* t[i])+ \
              pulp.lpSum((((x[i,j])-(y1))*t[i]) for j in set_J) <= C   #6#
    Model += ((y1)-a) >= (-1.5)  #7#
    Model += ((1.5*y1)-a) <= 0  #8#
